...
namespace Engine
{
	...
	//
	// Every operation throws zmq::error_t on genuine failure — cppzmq does this itself,
	// so failures are never silently swallowed here. TryReceive() is the one deliberate
	// exception: it returns std::nullopt for the expected "no message available yet"
	// case, and still throws for any other error.
	//
